...
const showFileNota = (sourceFile) => {
    const targetView = document.getElementById("target-view");
    targetView.setAttribute('src', sourceFile);
    $("#preview-nota").modal('show');
}

const filterNotaSupplier = (el) => {
    const supplierId = el.value;
    const dashboardPageURL = `${BASE_URL}dashboard`;
    if(supplierId !== 'ALL' && supplierId !== '') {
        window.location = dashboardPageURL + `?id_supplier=${supplierId}#nota-supplier`;
    } else {
        window.location = dashboardPageURL + '#nota-supplier';
    }
}

const filterNotaSales = (el) => {
    const salesId = el.value;
    const dashboardPageURL = `${BASE_URL}dashboard`;
    if(salesId !== 'ALL' && salesId !== '') {
        window.location = dashboardPageURL + `?id_sales=${salesId}#nota-penjualan`;
    } else {
        window.location = dashboardPageURL + '#nota-penjualan';
    }
}